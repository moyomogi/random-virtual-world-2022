<template>
  <client-only>
    ...
    <article class="container px-5 py-4 mx-auto md:flex md:flex-wrap">
      <section
        v-for="(slides, genre) in slidesList"
        :key="genre"
        class="mx-auto p-4 h-full md:w-1/2 lg:w-1/3 md:flex md:flex-col"
      >
        <!-- class="mx-auto p-4 h-full md:w-1/2 lg:w-1/3 md:flex md:flex-col" -->
        <section class="p-4">
          <img
          ...
          :class="colorsDict[genresDict[genre].color].borderClass"
            class="h-full bg-white border-2 rounded-lg overflow-hidden"
              ...
              <div class="p-6">
                <div class="items-baseline">
                  ...
                  >
                    {{ slide.title }}
                    <span
                      :class="[colorsDict[genresDict[genre].color].textClass, colorsDict[genresDict[genre].color].bgClass]"
                      class="
                        inline-block
                        p-2
                        text-xs
